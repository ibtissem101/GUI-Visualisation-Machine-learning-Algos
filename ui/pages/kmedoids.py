...
class KMedoidsPage(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        subtitle = ctk.CTkLabel(
            self,
            text="A partitioning algorithm that uses actual data points (medoids) as cluster centers, making it robust to outliers.",
            font=("Segoe UI", 14),
            text_color="#64748B",
            anchor="w"
        )
    ...
    def run_kmedoids(self):
        ...
        thread = threading.Thread(target=self._run_kmedoids_thread, args=(df, k, max_iter, selected_features))
        thread.daemon = True
        thread.start()

    def _run_kmedoids_thread(self, df, k, max_iter, selected_features):
        try:
            ...
            # Use KMeans then find closest actual points as medoids
            ...
            medoids_scaled = X_scaled[closest]
            ...
            if len(selected_features) > 2:
                ...
                medoids_viz = pca.transform(medoids_scaled)
            ...
            self.after(0, lambda: self._finish_kmedoids(
                X_viz.copy(), labels.copy(), medoids_viz.copy(), 
                list(viz_labels), sil_score, len(selected_features), explained_var
            ))
        except Exception as e:
            self.after(0, lambda err=str(e): self._handle_error(err))

    def _finish_kmedoids(self, X, labels, medoids, viz_labels, sil_score, n_features, explained_var):
        ...
        if sil_score > 0.5:
            self.insights_text.insert("end", "Interpretation: The clusters are well-separated and dense.\n")
             ...
             self.insights_text.insert("end", "Interpretation: The clusters are overlapping. Consider changing k.\n")
        
        self.insights_text.configure(state="disabled")
        
        self.plot_results(X, labels, medoids, viz_labels, sil_score)
    ...
    def plot_results(self, X, labels, medoids, viz_labels, sil_score):
        if self.canvas:
            self.canvas.get_tk_widget().destroy()
