...
class EDAPage(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(header_frame, text="Exploratory Data Analysis", font=("Segoe UI", 28, "bold"), text_color="#111827", anchor="w").pack(anchor="w")
        ctk.CTkLabel(header_frame, text="Visualize and explore your dataset with statistical summaries and charts", font=("Segoe UI", 14), text_color="#6B7280", anchor="w").pack(anchor="w", pady=(6, 0))
        
        ctk.CTkFrame(self, fg_color="#E5E7EB", height=1).pack(fill="x", padx=40, pady=(25, 30))
        
        content = ctk.CTkFrame(self, fg_color="transparent")
        content.pack(fill="both", expand=True, padx=40, pady=(0, 40))
        
        # Left Panel
        controls_panel = ctk.CTkFrame(content, fg_color="white", corner_radius=12, border_width=1, border_color="#E5E7EB")
        ...
        ctk.CTkLabel(controls_inner, text="Visualizations", font=("Segoe UI", 16, "bold"), text_color="#111827", anchor="w").pack(fill="x", pady=(0, 20))
        ctk.CTkLabel(controls_inner, text="Chart Type", text_color="#6B7280", font=("Segoe UI", 12), anchor="w").pack(fill="x", pady=(0, 10))
        
        self.chart_var = ctk.StringVar(value="Statistics Summary")
        chart_types = ["Statistics Summary", "Distribution Plots", "Correlation Heatmap", "Box Plots", "Scatter Matrix", "Missing Data", "Pairwise Correlations"]
        
        self.chart_menu = ctk.CTkOptionMenu(controls_inner, values=chart_types, variable=self.chart_var, fg_color="#2563EB", button_color="#2563EB", button_hover_color="#1D4ED8", dropdown_fg_color="white", dropdown_hover_color="#F3F4F6", dropdown_text_color="#111827", height=40, font=("Segoe UI", 12), corner_radius=8)
        ...
        ctk.CTkLabel(info_inner, text="ⓘ", font=("Segoe UI", 16), text_color="#2563EB").pack(side="left", padx=(0, 8))
        ctk.CTkLabel(info_inner, text="Select a visualization type\nto explore your data.", text_color="#1E40AF", font=("Segoe UI", 11), justify="left").pack(side="left", anchor="w")
        ...
        self.status_label = ctk.CTkLabel(controls_inner, text="", text_color="#6B7280", font=("Segoe UI", 11))
        
        self.generate_btn = ctk.CTkButton(controls_inner, text="Generate Visualization", command=self.generate_viz, font=("Segoe UI", 14, "bold"), fg_color="#2563EB", hover_color="#1D4ED8", height=46, corner_radius=8)
        self.generate_btn.pack(fill="x")
        
        # Right Panel
        self.viz_panel = ctk.CTkScrollableFrame(content, fg_color="white", corner_radius=12, border_width=1, border_color="#E5E7EB")
    ...
    def plot_placeholder(self):
        ...
        placeholder = ctk.CTkFrame(self.viz_panel, fg_color="transparent")
        placeholder.pack(expand=True, fill="both")
        inner = ctk.CTkFrame(placeholder, fg_color="transparent")
        inner.place(relx=0.5, rely=0.5, anchor="center")
        
        icon_bg = ctk.CTkFrame(inner, fg_color="#F3F4F6", width=80, height=80, corner_radius=40)
        icon_bg.pack(pady=(0, 20))
        icon_bg.pack_propagate(False)
        ctk.CTkLabel(icon_bg, text="◇", font=("Segoe UI", 36), text_color="#9CA3AF").place(relx=0.5, rely=0.5, anchor="center")
        
        ctk.CTkLabel(inner, text="No Visualization Generated", text_color="#111827", font=("Segoe UI", 18, "bold")).pack(pady=(0, 8))
        ctk.CTkLabel(inner, text="Load data and select a visualization type\nto see exploratory analysis charts", text_color="#6B7280", font=("Segoe UI", 13), justify="center").pack()
    ...
    def show_statistics(self, df):
        ...
        if len(numeric_df.columns) > 0:
            ctk.CTkLabel(stats_frame, text="Numeric Features Summary", font=("Segoe UI", 16, "bold"), text_color="#111827", anchor="w").pack(fill="x", pady=(0, 16))
            stats = numeric_df.describe()
            self.create_stats_table(stats_frame, stats)
        
        cat_df = df.select_dtypes(include=['object'])
        if len(cat_df.columns) > 0:
            ctk.CTkLabel(stats_frame, text="Categorical Features Summary", font=("Segoe UI", 16, "bold"), text_color="#111827", anchor="w").pack(fill="x", pady=(24, 16))
    ...
    def create_stats_table(self, parent, stats):
        ...
        for i, header in enumerate(headers):
            ctk.CTkLabel(table_inner, text=header if i == 0 else (header[:20] + "..." if len(header) > 20 else header), font=("Segoe UI", 11, "bold"), text_color="#374151", anchor="w").grid(row=0, column=i, padx=12, pady=12, sticky="w")
        
        for i, idx in enumerate(stats.index):
            ctk.CTkLabel(table_inner, text=idx, font=("Segoe UI", 11), text_color="#6B7280", anchor="w").grid(row=i+1, column=0, padx=12, pady=8, sticky="w")
            for j, col in enumerate(stats.columns):
                val = stats.loc[idx, col]
                ctk.CTkLabel(table_inner, text=f"{val:.3f}" if not pd.isna(val) else "N/A", font=("Segoe UI", 11), text_color="#4B5563", anchor="w").grid(row=i+1, column=j+1, padx=12, pady=8, sticky="w")
    ...
    def create_value_count_card(self, parent, column_name, value_counts):
        ...
        ctk.CTkLabel(card, text=column_name, font=("Segoe UI", 13, "bold"), text_color="#111827", anchor="w").pack(padx=18, pady=(14, 8), anchor="w")
        for val, count in value_counts.items():
            val_str = str(val)[:50] + "..." if len(str(val)) > 50 else str(val)
            ctk.CTkLabel(card, text=f"  •  {val_str}: {count}", font=("Segoe UI", 11), text_color="#6B7280", anchor="w").pack(padx=18, pady=3, anchor="w")
    ...
    def _create_boxplots(self, df):
        ...
        for i, col in enumerate(numeric_df.columns[:6]):
            ax = fig.add_subplot(n_rows, 2, i+1)
            data = numeric_df[col].dropna()
            ax.boxplot(data, vert=True, patch_artist=True, boxprops=dict(facecolor='#2563EB', alpha=0.7), medianprops=dict(color='#1E40AF', linewidth=2))
        ...
        for i in range(n):
            for j in range(n):
                ...
                if i == j:
                    ...
                    ax.scatter(numeric_df[cols_to_use[j]], numeric_df[cols_to_use[i]], alpha=0.4, s=15, color='#2563EB')
    ...
    def _create_missing_data(self, df):
        """Visualize missing data including placeholder values"""
        # Count both null values and placeholder values
        missing_counts = {}
        ...
        # Sort by count
        missing = pd.Series(missing_counts).sort_values(ascending=False)
        ...
        for i, (bar, val) in enumerate(zip(bars, missing.values)):
            ax.text(val + max(missing.values)*0.02, i, str(val), va='center', fontsize=10, fontweight='bold')
        
        ax.set_yticks(y_pos)
        ax.set_yticklabels(missing.index, fontsize=11)
        ax.set_xlabel('Number of Missing/Empty Values', fontsize=12, fontweight='bold')
        ax.set_title('Missing Data by Column (including ?, N/A, NULL, etc.)', fontsize=14, fontweight='bold', pad=15)
    ...
    def _create_pairwise_corr(self, df):
        ...
        for i, (bar, val) in enumerate(zip(bars, values)):
            ...
            ax.text(x_pos, i, f'{val:.3f}', va='center', ha='left' if val > 0 else 'right', fontsize=9, fontweight='bold')
